...
void field_state_handler(Command_t *cmd, size_t arg_idx) {
	...
	//cmd->where_args.args_count =0;
	//printf("This many args: %ld\n", cmd->args_len);
    while(arg_idx < cmd->args_len) {
		//printf("Currently parsing %ld argument, value is %s\n", arg_idx, cmd->args[arg_idx]);
...
void where_state_handler(Command_t *cmd, size_t arg_idx){
	/*seeing how many where conditions there are*/
	cmd->hasWhere =1;
