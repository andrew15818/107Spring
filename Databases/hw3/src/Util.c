...
void print_users(Table_t *table, int *idxList, size_t idxListLen, Command_t *cmd) {
	...
	//printf("Currently, there are %d where args\n", cmd->where_args.args_count);
    if (offset == -1) {
        offset = 0;
    }
...
///
/// This function parses wheres and
/// prints corresponding users
///
void print_wheres(Table_t *table, Command_t *cmd){

	for(int i=0;i<table->len;i++){
		int met =0;
		User_t *usr =get_User(table, i);
		for(int j =0; j< cmd->args_len;j++){
			if(!strncmp(cmd->args[j]"="))
		}
	}
	return ;
}
